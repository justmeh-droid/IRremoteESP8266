...
String IRGoodweatherAc::toString(void) {
  ...
  switch (this->getSwing()) {
    case kGoodweatherSwingFast: result += F(" (Fast)"); break;
    case kGoodweatherSwingSlow: result += F(" (Slow)"); break;
    case kGoodweatherSwingOff:  result += F(" (Off)"); break;
    default:                    result += F(" (UNKNOWN)");
  }
  result += addIntToString(getCommand(), F("Command"));
  switch (this->getCommand()) {
    case kGoodweatherCmdPower:    result += F(" (Power)"); break;
    case kGoodweatherCmdMode:     result += F(" (Mode)"); break;
    case kGoodweatherCmdUpTemp:   result += F(" (Temp Up)"); break;
    case kGoodweatherCmdDownTemp: result += F(" (Temp Down)"); break;
    case kGoodweatherCmdSwing:    result += F(" (Swing)"); break;
    case kGoodweatherCmdFan:      result += F(" (Fan)"); break;
    case kGoodweatherCmdTimer:    result += F(" (Timer)"); break;
    case kGoodweatherCmdAirFlow:  result += F(" (Air Flow)"); break;
    case kGoodweatherCmdHold:     result += F(" (Hold)"); break;
    case kGoodweatherCmdSleep:    result += F(" (Sleep)"); break;
    case kGoodweatherCmdTurbo:    result += F(" (Turbo)"); break;
    case kGoodweatherCmdLight:    result += F(" (Light)"); break;
    default:                      result += F(" (UNKNOWN)");
  }
  return result;
}
